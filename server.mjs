    ...
    try {
      ...
      if (lang) {
        ...
        try {
          ...
          // If language not available, extract available languages and try the first one
        ...
        // No language specified, let youtube-transcript pick the default
        transcript = await YoutubeTranscript.fetchTranscript(videoId)
        usedLang = transcript[0]?.lang || 'unknown'
      }

      // Determine track info from first item
      const trackLang = transcript[0]?.lang || usedLang

      // Transform to our format: { start (seconds), duration (seconds), text }
